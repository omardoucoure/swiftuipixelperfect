export async function extractListData(node) {
    let extractedData = [];
...
async function getListProperties(node) {
    let properties = {};
...
function findButtonInListItem(node) {
    if ("children" in node && Array.isArray(node.children)) {
        for (const child of node.children) {
...
function getIconFromListItem(listItem) {
